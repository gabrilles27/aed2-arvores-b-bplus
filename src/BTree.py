...
class BTree:
    ...
    # Adicione este bloco dentro da class BTree, logo abaixo do __init__ ou insert
    def search(self, x, k):
        i = 0
        # Procura a primeira chave maior ou igual a k
        # x.keys[i][0] acessa o ID (ISBN) dentro da tupla (chave, offset)
        while i < len(x.keys) and k > x.keys[i][0]:
            i += 1

        # Se encontrou a exata chave neste nó, retorna o offset (posição 1 da tupla)
        if i < len(x.keys) and x.keys[i][0] == k:
            return x.keys[i][1]

        # Se a chave não foi encontrada e o nó é uma folha, o registro não existe
        if x.leaf:
            return None

        # Caso contrário, desce recursivamente para o nó filho apropriado
    ...
    # Busca de intervalo (Range Search) para a Árvore B normal
    def range_search(self, x, start_id, end_id, results=None):
        # Inicializa a lista na primeira chamada recursiva
        if results is None:
            results = []
        ...
        i = 0
        # 1. Encontra o índice da primeira chave que NÃO é menor que start_id
        while i < len(x.keys) and x.keys[i][0] < start_id:
            i += 1

        # 2. Desce para a subárvore à esquerda dessa chave,
        # pois ela pode conter valores no limite inicial do intervalo
        if not x.leaf:
            self.range_search(x.child[i], start_id, end_id, results)

        # 3. Percorre as chaves do nó atual e processa as subárvores à direita
        while i < len(x.keys) and x.keys[i][0] <= end_id:

            # Se a chave atual está no intervalo, adiciona o offset à lista
            if x.keys[i][0] >= start_id:
                results.append(x.keys[i][1])

            # Desce para a subárvore à direita da chave atual
            if not x.leaf:
                self.range_search(x.child[i + 1], start_id, end_id, results)
